...
LATEST_VERSION=$(curl --silent "https://raw.githubusercontent.com/RicePudding0xL/43cr34rc/main/package.json" | grep '"version":' | sed -E 's/.*"([^"]+)".*/\1/')
...
function installAll() {
  ...
  git clone https://github.com/RicePudding0xL/43cr34rc 43cr34rc &> /dev/null
  cd nitro-sniper
